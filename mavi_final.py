...
Valid examples include battle mechanics, player stats, enemy behavior, actions,
resources, rewards, items, progression, or risk/reward mechanics.
...
Return the COMPLETE sari_sistem.py file in the content field.
...
"""
    ...
    for model in models:
        try:
            ...
            last_error = exc
            print(
                f"Rate limit: {model}. Fallback model deneniyor...",
                file=sys.stderr,
            )
            continue
    ...
    if response is None:
        print(
            "HATA: Mavi hiçbir kullanılabilir modelden cevap alamadı.",
            file=sys.stderr,
        )
    ...
    if not isinstance(content, str):
        raise ValueError(
            "Mavi geçerli source code üretmedi."
        )
    ...
    if len(content) < 300:
        raise ValueError(
            "Mavi tarafından üretilen kod çok kısa."
        )
