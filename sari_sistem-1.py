...
class Game:
    ...
    def _enemy_turn(self) -> None:
        ...
        p = self.config["player"]
        raw_damage = max(0, int(self.battle.enemy["attack"]) - int(p["base_defense"]))
...
def write_generation(value: int) -> None:
    COUNTER_FILE.write_text(str(max(0, int(value))) + "\n", encoding="utf-8")
...
def self_test() -> dict[str, Any]:
    ...
    game4.start_battle(seed=0)
    assert game4.buy_potion() is False
    ...
    return {
        ...
        "checks": [
            ...
            "heal restores hp",
            "guard creates a real shield and reduces damage",
